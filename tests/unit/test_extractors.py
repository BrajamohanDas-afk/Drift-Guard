...
def test_url_found():
    assert "https://grafana.internal/d/payments" in extract_urls(
        "Check https://grafana.internal/d/payments here"
    )
...
def test_dashboard_found():
    assert "grafana:payments-overview" in extract_dashboards(
        "Monitor at grafana:payments-overview"
    )
...
def test_helm_chart_found():
    assert "bitnami/postgresql@12.1.0" in extract_helm_charts(
        "Deploy bitnami/postgresql@12.1.0"
    )
...
def test_dependency_label_found():
    assert extract_dependencies("Dependencies: postgres, redis and kafka") == [
        "postgres",
        "redis",
        "kafka",
    ]
...
def test_dependency_inline_found():
    assert extract_dependencies("payments-api depends on postgres and redis") == [
        "postgres",
        "redis",
    ]
...
def test_dependency_not_found():
    assert extract_dependencies("restart payments-api now") == []


def test_entity_extractor_includes_dependency_entities():
    ...
    dependency_values = {
        ...
        if entity["entity_type"] == "dependency"
    }
    assert dependency_values == {"postgres", "redis"}
